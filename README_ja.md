...
- **タスク判定** — タスク・エネルギーレベル・利用可能時間を入力すると、AI が最適な順序とアプローチを提案
- **比較判定** — 複数の選択肢を AI が多角的に比較し、メリット・デメリットとスコアリングを生成
- **羅針盤（Compass）** — テキスト/URL/画像で目標・夢・価値観を蓄積。AI が判定時に参照し、あなたにとって大切なことに沿った意思決定を支援
- **ドキュメント RAG** — PDF やテキストファイルをアップロード。AI が判定時のコンテキストとして活用
